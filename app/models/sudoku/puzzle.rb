...
class Sudoku::Puzzle
    ...
    def initialize(values: nil, options: nil, source_matrix: nil)
        ...
        @matrix = Matrix.build(9) do |ci, cj|
            ...
            Sudoku::Cell.new(puzzle: self, value: value, ci: ci, cj: cj, options: cell_options)
        end

        @id = @@sqids.encode([ Time.now.to_i ])

        @collections_cache = Sudoku::SudokuCache.new({
    ...
    def duplicate
        Sudoku::Puzzle.new(source_matrix: self)
    end
    ...
    def validate_source_matrix(source_matrix)
        ...
        unless source_matrix.is_a?(Sudoku::Puzzle)
    ...
    def build_rows
      ...
      Sudoku::COORD_RANGE.each do |ci|
        rows_hash[ci] = Sudoku::Row.new(@matrix.row(ci).to_a)
      end
      rows_hash
    end
    ...
    def build_columns
      ...
      Sudoku::COORD_RANGE.each do |cj|
        columns_hash[cj] = Sudoku::Column.new(@matrix.column(cj).to_a)
      end
      columns_hash
    end
    ...
    def build_blocks
        ...
        [  [ 0, 0 ], [ 0, 1 ], [ 0, 2 ],
           ...
           [ 2, 0 ], [ 2, 1 ], [ 2, 2 ] ].each do |coords|
            blocks_hash[coords] = Sudoku::Block.new(self, coords[0], coords[1])
        end
        blocks_hash
    end
