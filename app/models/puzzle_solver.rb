class PuzzleSolver
    ...
    def initialize(puzzle, isolate: false, display: false, print: false, slow_display: false)
    ...
    def forbid_other_cells_in_block_for_family(value: nil, values: [], family:)
        ...
        block.cells.each do |cell|
            ...
            if value.present?
                cell.forbid(value, display_speed: @display_speed)
            elsif values.present?
                cell.forbid_multiple(values, display_speed: @display_speed)
            else
    ...
    def handle_puzzle_complete
        if @puzzle.complete?
            print_to_console do
                puts "Puzzle solved in #{time_solving} seconds!"
                @puzzle.print
            end
            @display_speed = "fast"
            return true
