class Sudoku
    ...
    def self.broadcast_cell(cell, display: false, display_speed: nil)
        ...
        Turbo::StreamsChannel.broadcast_action_to(:cell_squares, action: :replace, target: "cell_#{cell.ci}_#{cell.cj}", partial: "sudoku/cell_square", locals: { value: cell.value, ci: cell.ci, cj: cell.cj, options: cell.options, show_options: show_options })
        case display_speed
        when "slow"
            sleep(0.1)
        when "medium"
        ...
        if display
          print display_speed == "slow" ? "🐢" : display_speed == "medium" ? "🐇" : "⚡️"
        end
    end
end
