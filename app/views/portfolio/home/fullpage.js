...
(function (global, factory) {
  ...
  function onDestroyAll$1() {
    ...
    win.history.replaceState(
      null,
      "",
      win.location.pathname + window.location.search,
    );
  }
  /**
  ...
  function slideArrowHandler() {
    ...
    var isPrevArrow =
      hasClass(this, SLIDES_PREV) || closest(this, SLIDES_PREV_SEL);
    /*jshint validthis:true */
  ...
  function getTmpPosition(v) {
    return hasClass(getState().activeSection.item, AUTO_HEIGHT)
  ...
  function getDestinationPosForInfiniteScroll(v) {
    ...
    if (v.isMovementUp && hasClass(v.element, AUTO_HEIGHT)) {
      return (
        getDestinationPosition(v.element) -
        getWindowHeight() +
  ...
  function createInfiniteSections(v) {
    ...
    if (!v.isMovementUp) {
      ...
      var prevSectionsReversed = prevAll(
        activeSectionItem,
        SECTION_SEL,
      ).reverse();
  ...
  function continuousVerticalFixSectionOrder(v) {
    ...
    if (v.isMovementUp) {
      ...
      after(
        ...
        prevAll(v.element, SECTION_SEL).reverse(),
      );
    }
    ...
    bindEvents: function bindEvents(item) {
      ...
      if (scrollable) {
        scrollable.addEventListener(
          "scroll",
          scrollOverflowHandler.onPanelScroll,
        );
      }
  ...
  function updateState() {
    ...
    state.sections.map(function (section) {
      ...
      if (section.slides.length) {
        ...
        if (
          !section.slides.find(function (slide) {
            return slide.isActive;
          })
        ) {
          // RTL: start from last slide, otherwise start from first
          var defaultSlide =
            getOptions().rtl && section.slides.length > 0
  ...
  var SectionPanel = function SectionPanel(el) {
    ...
    var defaultSlide =
      getOptions().rtl && this.slides.length > 0
  ...
  function styleSlides(section) {
    ...
    if (
      ...
      var defaultSlideIndex =
        getOptions().rtl && slidesElems.length > 0 ? slidesElems.length - 1 : 0;
  ...
  function MouseWheelHandler(e) {
    ...
    if (getOptions().autoScrolling && !getControlPressed() && !isNormalScroll) {
      ...
      var horizontalDetection =
        Math.abs(horizontalValue) > 0 || Math.abs(deltaX) > 0; // @ts-ignore - cross-browser compatibility

      var isScrollingVertically =
        !isShiftPressed &&
        (Math.abs(deltaX) < Math.abs(deltaY) || !horizontalDetection);
      ...
      var horizontalDelta = isShiftPressed
        ? delta
        : Math.max(-1, Math.min(1, horizontalValue));
      var horizontalDirection = isScrollingHorizontally
        ? horizontalDelta < 0
          ? "right"
          : horizontalDelta > 0
            ? "left"
            : "none"
        : "none"; //is scroll allowed for the detected direction?
      ...
      if (isScrollingVertically) {
        ...
        var horizontalScrollValue = isShiftPressed
          ? Math.abs(value)
          : Math.abs(horizontalValue);
      ...
      if (state.canScroll) {
        ...
        // SHIFT+wheel doesn't require acceleration (intentional gesture)
          ...
          isScrollingHorizontally &&
          (isShiftPressed || isAccelerating(horizontalScrollings))
        ) {
          setState({
            scrollTrigger: "wheel",
          });
  ...
  function anchorLinkClickHandler(e) {
    ...
    var slide =
      slideEl && sectionEl.contains(slideEl)
        ? getPanelByElement(section.slides, slideEl)
        : null; // Prevent default browser scroll behavior
