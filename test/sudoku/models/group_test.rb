...
class GroupTest < Minitest::Test
  def setup
    @puzzle = Sudoku::Puzzle.new(values: Matrix.build(9) { 0 }, options: Matrix.build(9) { Set.new(1..9) })
  ...
  def test_initialize_valid
    ...
    assert @group.cells.all? { |c| c.is_a?(Sudoku::Cell) }, "All group members should be Cell objects"
    assert_equal [ 1, 2, 0, 0, 0, 0, 0, 0, 0 ], @group.values, "Group should store the correct values from cells"
  end

  def test_initialize_invalid
    assert_raises(ArgumentError, "Should raise error for non-Cell array") { Sudoku::Group.new([ 1, 2, 3 ]) }
    assert_raises(ArgumentError, "Should raise error for invalid Cell array length") { Sudoku::Group.new(Array.new(4) { |i| Sudoku::Cell.new(puzzle: @dummy_puzzle, value: i, ci: 0, cj: i, options: Set.new([ 1, 2, 3 ])) }) }
    assert_raises(ArgumentError, "Should raise error for non-array input") { Sudoku::Group.new("not an array") }
  end
