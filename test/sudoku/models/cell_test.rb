...
class CellTest < Minitest::Test
  def setup
    ...
    @puzzle = Sudoku::Puzzle.new(values: Matrix[*@valid_puzzle_array])
    @cell = @puzzle.cell(1, 2)
  end

  def test_initialize_valid
    cell = Sudoku::Cell.new(puzzle: @puzzle, value: 5, ci: 3, cj: 4, options: Set.new([ 1, 2, 3 ]))
  ...
  def test_cell_knows_its_puzzle
    puzzle = Object.new
    cell = Sudoku::Cell.new(puzzle: puzzle, value: 1, ci: 0, cj: 0)
  ...
  def test_initialize_invalid_value
    assert_raises(ArgumentError, "Should raise error for value > 9") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 10, ci: 1, cj: 1) }
    assert_raises(ArgumentError, "Should raise error for value < 0") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: -1, ci: 1, cj: 1) }
  end

  def test_initialize_invalid_indices
    assert_raises(ArgumentError, "Should raise error for ci > 9") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 1, ci: 10, cj: 1) }
    assert_raises(ArgumentError, "Should raise error for cj > 9") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 1, ci: 1, cj: 10) }
  end

  def test_initialize_invalid_options
    assert_raises(ArgumentError, "Should raise error for options containing 0") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 0, ci: 1, cj: 1, options: Set.new([ 0, 1, 2 ])) }
    assert_raises(ArgumentError, "Should raise error for options containing > 9") { Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 0, ci: 1, cj: 1, options: Set.new([ 1, 2, 10 ])) }
  end
  ...
  def test_block_i_j_coordinates
    cell = Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 0, ci: 4, cj: 7)
  ...
  def test_siblings
    @valid_puzzle = Sudoku::Puzzle.new(values: Matrix[*@valid_puzzle_array])
    cell = @valid_puzzle.cell(0, 0)
    siblings = cell.siblings
    assert siblings.is_a?(Set), "siblings should return a set"
    assert siblings.all? { |s| s.is_a?(Sudoku::Cell) }, "All siblings should be Cell instances"
  ...
  def test_inspect
    cell = Sudoku::Cell.new(puzzle: @dummy_puzzle, value: 5, ci: 3, cj: 4)
